...
\subsubsection{Mechanische Planung}
    ...
    \begin{figure}[h]
        \centering
        \vspace{-10px}
        \includegraphics[width=0.7\textwidth]{Vogis Bilder/Beispielsmodul_in_AUTOCAD.png}
        \caption{Beispielbild aus AutoCAD}
        \vspace{-10px}
        \label{fig:AutoCAD-Beispielbild}
    \end{figure}
...
\subsubsection{Elektrische Planung (E-Plan)}
    ...
    Auf den nächsten beiden Seiten finden sich jweils eine 8A-DC-Sicherung. Eingehend sind die bereits genannten Brücken, diese gehen dann auch weiter zur nächsten Sicherung, und ausgehend sind einmal + und - für die stärkeren Schrittmotoren. Wie erwähnt sind das Seite 5 und 6.\\\\
    Auf Seite 7 findet sich eine 2A-DC-Sicherung für einen von drei schwächeren Schrittmotor. Diese hat ebenfalls die Brückenverbindung eingehend, allerdings hört die Brückenverbindung mot dieser Sicherung auf. Das bedeutet ein Feed-In-Modul versorgt drei Sicherungen. Abgehend hat diese Sicherung + und - für den passenden Schrittmotor.\\\\
    
    Ursprünglich waren die drei netzteile , dnan kanmmm asi
